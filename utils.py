...
# 2.2 final：提示詞打包技術面摘要 helper
# 放在 utils.py，供 ui_main.py 透過 ui_common import * 使用。
def build_technical_summary(hist, lookback=120):
    ...
    try:
        ...
        close_non_na = close.dropna()
        if close_non_na.empty:
            return {"available": False, "summary_text": "", "error": "Close 無有效數值"}
        last_close = float(close_non_na.iloc[-1])

        def ma(n):
            if len(close_non_na) < n:
                return None
        ...
        dev5_str = "NULL" if dev5 is None else f"{dev5:.2f}"
        dev20_str = "NULL" if dev20 is None else f"{dev20:.2f}"
        ...
        summary = (
            ...
            f"- 沿線上攻: {along_text}\n"
            f"- 乖離與追價風險: 距 5MA={dev5_str}%；距 20MA={dev20_str}%；{dev_note}\n"
            f"- KD 狀態: {kd_text}\n"
            f"- 量價結構: {vol_text}\n"
            f"- 支撐平台: {support_text}\n"
