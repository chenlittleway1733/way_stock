...
def render_main_page(sidebar_state=None):
    ...
    if curr_id:
        ...
        if hist is not None and not hist.empty:
            ...
            with col_eps2:
                ...
                if dynamic_cap_pack.get("available") and dynamic_cap_pack.get("final_cap") is not None:
                    ...
                    if _cap_low is not None and _cap_high is not None:
                        cap_reason = f"Dynamic Cap 2.0 可操作倍率：{float(_cap_low):.1f}～{float(_cap_high):.1f}x；中性建議 {suggested_cap:.1f}x。已採 17-C-6：AI 校對後採用值 + 分歧折扣校準 + 循環復甦區間 + 產業 hard ceiling。"
                    else:
                        cap_reason = f"Dynamic Cap 2.0 最終建議倍率：{suggested_cap:.1f}x。已採 17-C-6：AI 校對後採用值 + 分歧折扣校準 + 循環復甦區間 + 產業 hard ceiling。"
                else:
                ...
                target_pe_cap = st.number_input(
                    ...
                    help="第 17-C-6：重構產業基準倍率，加入市場/法人隱含倍率、負 EPS 防呆與題材落地檢查。"
                )
            ...
            # 17-C-6：倍率分層。可操作倍率 ≠ 公式合理倍率 ≠ 樂觀/極限倍率。
            ...
            # 17-C-6：市場 / 法人隱含 Forward P/E 對照。用來解釋現價、法人價與系統估值差距。
                ...
                if isinstance(dynamic_cap_pack, dict) and dynamic_cap_pack.get("report") is not None:
                    with st.expander("⚙️ Dynamic Cap 2.0 倍率拆解", expanded=True):
                        if dynamic_cap_pack.get("valuation_mode") == "pb_cycle":
                            ...
                            st.caption("17-C-6：已加入循環復甦判斷、分歧折扣校準、公式/可操作/樂觀倍率分離，最後才套用產業 hard ceiling。")
